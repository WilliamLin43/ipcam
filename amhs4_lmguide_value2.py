...
def get_value2(frame):
    ...
    #v2.imshow("Particle Detection Image", np.hstack([img1])) #show drawing
    #cv2.waitKey(0)                 #等待按下任何按鍵
    ...
    value2 = round((np.sum(draw_img)/total_area)*100,4)
    ...
    return draw_img,value2


if __name__ == '__main__':
    filename = cv2.imread("./cut0.png")
    frame = cv2.imread(filename)
    print(get_value2(frame))
