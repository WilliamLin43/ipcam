...
Set_VALUE1=fp.VALUE1.astype(float)
Set_VALUE2=fp.VALUE2.astype(float)
...
while(True):
    ...
    for i in range(len(IP)):
        folderpath = "./InputImage/"+str(DEVICE_ID[i])+"_"+str(DEVICE_NUMBER[i]) # Check folder exist or not.
        print(folderpath)
        if os.path.isdir(folderpath):
            ...
            for file in files: #遍歷資料夾
                ...
                if not os.path.isdir(file): #Check if not folder, open it
                    #Check image files writed time
                    CheckfileName=time.strftime("%Y%m%d%H%M%S", time.localtime())
                    CheckfileNameValue=int(CheckfileName)
                    print(CheckfileNameValue-5)
                    CheckfileName2=file[:14]
                    CheckfileNameValue2=int(CheckfileName2)
                    print(CheckfileNameValue2)
                    print(CheckfileNameValue-CheckfileNameValue2)
                    if (CheckfileNameValue-CheckfileNameValue2)>10: #Delay 10 seconds
                        ...
                        for j in range(2):
                            ...
                            dataPath=r'./'+str(DEVICE_ID[i])+"_"+str(DEVICE_NUMBER[i])+'cutpoints'+str(j)+'.csv'
                            ...
                            Initial_Img = cv2.imread("./"+str(DEVICE_ID[i])+"_"+str(DEVICE_NUMBER[i])+"_cut"+str(j)+"-0.png") #Read initial image
                            ...
                            area=np.sum(gray1)
                            print(total_area)
                            print((np.sum(gray1)/total_area))
                            Value1=round((np.sum(gray1)/total_area)*100,4)
                            
                            cv2.putText(gray1,'Value:'+str(Value1)+'% / '+str(Set_VALUE1[i])+'%',(10,20),cv2.FONT_HERSHEY_SIMPLEX,0.5,(255,255,0),1,cv2.LINE_AA)
